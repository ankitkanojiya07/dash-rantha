/** Collapse category/status variants (case, punctuation, typos) into one canonical label. */
...
/** Match key: lowercase, punctuation → spaces, singularize last-ish words. */
...
export function categoriesShouldMerge(a: string, b: string): boolean {
  if (a === b) return true;
  ...
  return dist > 0 && dist <= maxEditDistance(maxLen) && dist / maxLen <= 0.3;
}
...
function pickCanonicalName(variants: Map<string, number>): string {
  ...
  const centrality = (name: string) => {
    ...
    return others.reduce((s, n) => s + levenshtein(key, categoryMatchKey(n)), 0) / others.length;
  };
...
export function buildCategoryCanonicalMap(names: string[]): Map<string, string> {
  ...
  for (let i = 0; i < unique.length; i++) {
    for (let j = i + 1; j < unique.length; j++) {
      if (categoriesShouldMerge(unique[i], unique[j])) {
...
export function canonicalizeCategory(name: string, map: Map<string, string>): string {
